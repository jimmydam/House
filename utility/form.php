...
<?php
	...
	if(!empty($_SESSION['currrentUser'])){
		...
		// sql SELECT command
		$command = "SELECT  ID, name, date, quantity FROM utility ORDER BY ID;";
...
<html>
    ...
    <body>
		...
		<?php 
			...
			<table>
				<tbody>					
					<tr><th class="theader"><b>ID<b/></th><th class="theader"><b>Name<b/></th><th class="theader"><b>Purchase Date<b/></th><th class="theader"><b> Quantity<b/></th></tr>					
					<?php
							// if user is logged on
							if(($_SESSION['logged_in'])){
								...
								for($x = 0; $x < $arrlength; $x++){
									...
									echo "<tr><td>$id[$x]</td> <td>$name[$x]</td> <td>$date[$x]</td><td>$quantity[$x]</td>" ;
									echo "<br>";
								}
							}
					?>		
